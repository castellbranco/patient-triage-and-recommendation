...
class ProviderRepository(BaseRepository):
    ...
    async def get_by_license(self, license_number: str):
        stmt = select(Provider).where(Provider.license_number == license_number, Provider.deleted_at.is_(None))
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def get_by_specialty(self, specialty: str):
        stmt = select(Provider).where(Provider.specialty == specialty, Provider.deleted_at.is_(None))
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def get_accepting_patients(self):
        stmt = select(Provider).where(Provider.accepting_new_patients == True, Provider.deleted_at.is_(None))
        result = await self.session.execute(stmt)
        return list(result.scalars().all())
