...
class Patient(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    blood_type: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Patient blood type"
    )

    address_line1: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Patient address line 1"
    )
    address_line2: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Patient address line 2 (Optional)"
    )
    city: Mapped[Optional[str]] = mapped_column(String(), nullable=True, doc="Patient city")
    postal_code: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Patient postal code (Optional)"
    )
    country: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Patient country (Optional)"
    )
    ...
    allergies: Mapped[dict] = mapped_column(
        ...
        server_default="[]",
        nullable=False,
        doc="List of allergies (e.g., [{'name': 'penicillin', 'severity': 'high'}])",
    )
    chronic_conditions: Mapped[dict] = mapped_column(
        JSONB,
        server_default="[]",
        nullable=False,
        doc="List of chronic conditions (e.g., [{'icd10': 'E11', 'name': 'Type 2 diabetes'}])",
    )
    medications: Mapped[dict] = mapped_column(
        JSONB,
        server_default="[]",
        nullable=False,
        doc="Current medications (e.g., [{'name': 'metformin', 'dosage': '500mg'}])",
    )
    emergency_contact: Mapped[dict] = mapped_column(
        JSONB,
        server_default="{}",
        nullable=False,
        doc="Emergency contact info (e.g., {'name': 'John Doe', 'phone': '+1234567890'})",
    )

    insurance_provider: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Health insurance provider (Optional)"
    )
    insurance_policy_number: Mapped[Optional[str]] = mapped_column(
        String(), nullable=True, doc="Health insurance policy number (Optional)"
    )

    user: Mapped["User"] = relationship(
        "User", back_populates="patient", lazy="selectin", doc="Associated user account"
    )
