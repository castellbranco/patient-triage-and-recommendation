...
async def test_create_provider_success(user_with_provider):
    ...
    provider = await user_with_provider("prov@example.com", specialty="Cardiology", license_number="LIC-123")
...
@pytest.mark.asyncio
async def test_provider_unique_license_constraint(db_session, user_with_provider, user_factory, provider_factory):
...
async def test_update_provider(db_session, user_with_provider):
    ...
    provider = await user_with_provider("update@example.com", specialty="General", accepting_new_patients=True)
...
async def test_provider_repr(user_with_provider):
    ...
    provider = await user_with_provider("repr@example.com", specialty="Neurology", license_number="LIC-REPR")
...
async def test_filter_providers_accepting_patients(db_session, user_with_provider):
    ...
    result = await db_session.execute(select(Provider).where(Provider.accepting_new_patients == True))
    assert len(result.scalars().all()) == 1
